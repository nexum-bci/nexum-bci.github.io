...
- **U3:** TPS7A4700RGWT — 超低噪声 3.3V LDO（模拟电源，噪声 1 µVrms）
...
| 参数 | 目标 | 布局影响 |
|------|------|---------|
| 输入参考噪声 | <1 µV RMS (0.5-100 Hz) | 开关噪声不得耦合进入模拟输入 |
...
- U3 (TPS7A4700，模拟 3.3V 稳压器) 必须在 U1 AVDD 引脚 20 mm 范围内。输出电容 C4 (10 µF) 必须在 U3 输出引脚 5 mm 范围内。
...
| 引脚 | 名称 | 连接 | 去耦 |
|------|------|------|------|
| 4 | VDD | 数字 3.3V（来自 nRF52 内部 LDOOUT） | 100 nF + 10 µF |
...
**关键：** nRF52 DC-DC 电感 (L1, 10 µH, 如 Murata LQM18PN100M00) 必须放置在引脚 18 (VDD) 和 19 (DEC4 — SW 输出) 的 5 mm 范围内。电感的交流电流路径经过 VDD 引脚 — 这里是大部分开关噪声辐射的位置。将电感放在 L1 上，其下方设专用过孔组（电感焊盘下 3 个过孔）。使该电感 **距离 ADS1299 至少 15 mm**。
...
| 器件 | 引脚 | 电容 | 布局规则 |
|------|------|------|---------|
| U1 (ADS1299) | AVDD（4 个引脚：5, 11, 20, 43） | 每引脚 100 nF + 10 µF 共用大电容 | 100 nF 距每个 AVDD 引脚 < 2 mm。10 µF 可在模拟区任意位置。 |
| U1 (ADS1299) | DVDD（引脚 42） | 100 nF + 10 µF | 100 nF 距引脚 42 < 2 mm |
| U1 (ADS1299) | VREFP（引脚 21） | 10 µF + 100 nF | 板上最近去耦。距引脚 < 2 mm。 |
| U1 (ADS1299) | VREFN（引脚 22） | 100 nF 至 GND | < 3 mm |
| U3 (TPS7A4700) | IN（引脚 26） | 10 µF + 100 nF | 距输入引脚 < 5 mm |
| U3 (TPS7A4700) | OUT（引脚 27） | 10 µF + 100 nF | 距输出引脚 < 5 mm。ESR 20-500 mOhm。 |
| U3 (TPS7A4700) | NR（引脚 14） | 10 µF（低漏电流） | 10 µF 至 GND。此电容设定内部基准噪声 — 使用 X7R 或薄膜电容，不要使用 X5R。 |
| U4 (TPS7A2005) | IN | 10 µF | < 5 mm |
| U4 (TPS7A2005) | OUT | 10 µF | < 5 mm |
| U6 (REF3125) | IN | 10 µF + 100 nF | < 5 mm |
| U6 (REF3125) | OUT | 10 µF | 距 U1 VREFP < 5 mm |
...
- **抗混叠滤波器：** 每通道，在连接器和 ADS1299 输入之间放置一个 2 阶无源 RC 低通滤波器（R = 1.5 kOhm, C = 1 µF, fc = 106 Hz）。这是 **模拟抗混叠滤波器** — 必须的。硬件：R11-R26（16 个电阻，0402）和 C11-C18（8 个电容，0603）。安装在连接器到 ADS1299 的 15 mm 布线走廊内。
