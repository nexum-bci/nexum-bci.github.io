...
| **端到端延迟（EEG → 运动）** | **< 185 ms**（原型验收 <200 ms） | **系统** |
...
| 系统重量（NeuroSuit + 控制盒） | < 1.5 kg（目标）/ < 2.5 kg（原型验收） | 康复穿戴容忍上限；量产通过碳纤维+织物持续减重 |

### 2.4 电源与电池

| 参数 | 目标 | 备注 |
|------|------|------|
| 电池容量 | 50 Wh（6S 锂离子，标称 22.2 V，2,250 mAh） | 21700 电芯 Samsung 50E 或同类；原型可用 18650 |
| 持续功耗 | ~8–12 W | MCU + BLE + IMU ~1 W；电机 + 驱动平均 ~7–11 W |
| 峰值功耗 | ~50 W（3 秒） | 电机峰值扭矩 + 加速 |
| 连续使用电池续航 | > 4 小时 | 一次康复训练（50 Wh / 12 W ≈ 4.2 h） |
| 充电时间 | < 2 小时 | USB-C PD（20 V / 3 A 输入，BQ25713 + BQ76952 BMS） |
| 电池类型 | 21700 锂离子（原型）；定制软包（量产） | 6S 配置，含均衡充电 BMS |

**关键权衡：** 电机电压 vs 电池节数。更高电压（36–48 V）可在相同功率下降低电流（更细导线、更少 I²R 损耗、更快电机响应），但需要更多串联电池（10S–12S → 更重）。推荐：**6S 锂离子（标称 22.2 V，3.7 V/节）**——平衡电机响应与电池组重量（50 Wh 约 360 g）。
...
**决定：6S 锂离子，21700 电芯**（Samsung 50S 2,500 mAh 高倍率或 Molicel P26A 2,600 mAh，6 × 2,500 mAh @ 22.2 V ≈ 55 Wh）。定制电池包，含 BMS（TI BQ76952 或类似）。
...
**总计：约 ¥200,000**（直接 BOM + 测试设备约 ¥93,900 + 应急储备金约 ¥93,300，用于 PCB 改版、机械重设计和意外费用）。剩余 ¥1,800,000 用于：6 个月团队工资、临床费用、办公/实验室空间和注册咨询。
...
| 数据包格式 | 8 通道 × 2 字节（16 位）+ 8 字节电极状态 + 1 字节电量 + 2 字节序号 = **每包 27 字节** |
| 数据包间隔 | 20 ms（50 Hz 更新） |
| 电极阻抗特征值 | `IMPEDANCE`（UUID：`EEG0-003`），读取，16 字节 |
| 命令特征值 | `CMD_RX`（UUID：`EEG0-004`），写入，3 字节 |
| 电池特征值 | 标准电池服务（UUID：`0x180F`） |
