...
可穿戴设备的机械架构存在根本性矛盾：刚性外骨骼（Ekso, HAL）力传输效率高但重12-25kg、关节压迫、穿戴耻感强。纯软体外骨骼（Harvard Wyss）轻便但55%的机械功率在软组织中损耗——电机发出100W，只有45W传到骨骼。

Kine采用**混合式架构**解决这个矛盾：

- **碳纤维髋部锚定**：力的起点。碳纤维比强度是钢的5倍、密度仅1.6g/cm³。3D打印成型，单件<$150（qty 100）。髋部是人体的"底盘"——从髋部传力到下肢，走最短的力传递路径，寄生扭矩最小
- **织物包裹**：力的分布面。压缩织物将锚定点的集中力分散为皮肤可舒适承受的面压力（<5 kPa，远低于医用止血带的安全阈值）。嵌入的16通道sEMG电极与外层织物一体编织，无需单独佩戴传感器绑带
- **Bowden电缆驱动**：力的传输路径。电缆模拟肌肉-肌腱的拉力路径——从髋部（起点）沿大腿外侧（力线）到膝关节上方（锚点）。电缆外鞘固定，内芯滑动，力只沿电缆方向传输，不产生横向寄生力。单个电缆组件$15-40

**力传输效率对比**：纯软体45%（Harvard实测）→ 混合架构>70%（碳纤维锚定+最短路径）→ 纯刚性>90%（但重12-25kg）。70%是穿戴体验与机械效率的最优平衡点。

**驱动电机**：AKE60-8一体化关节电机。260g，峰值扭矩12.5Nm，扭矩密度46Nm/kg。头部消费外骨骼通用方案——不自研电机，聚焦系统集成。qty 1,000单价约$150。

### 3.5 传感器融合架构

Kine的传感器不是堆料——它们按延迟和功能形成三层互补架构：

| 传感器 | 延迟 | 功能 | 失效模式 | 冗余 |
|--------|------|------|---------|------|
| EEG（8ch干电极） | 0.5-1.5s（预判） | 宏观意图分类（走/停/方向） | 出汗、运动伪迹 | sEMG+IMU可独立运行 |
| sEMG（16ch织物） | <100ms | 精细力矩解码 | 皮肤接触不良 | IMU可降级为姿态触发 |
| IMU（6轴×4） | <10ms | 步态相位、关节角度 | 漂移累积 | 足底压力交叉校验 |
| 足底压力（4点） | <5ms | 地面接触检测 | 鞋内位移 | IMU步态相位冗余 |

**安全设计**：任何单一传感器失效，系统降级运行而非停机。EEG失效→sEMG+IMU继续。sEMG失效→IMU触发基础助力。全传感器失效→无助力被动模式（等同于普通压缩裤）。
...
极壳、Ekso是硬件公司。BrainCo是BCI公司。**同时覆盖神经接口、数字孪生、RL智能、混合机械、传感器融合五层的，全球只有Kine。**
